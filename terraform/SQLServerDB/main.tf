            ...
            virtualNetworkRules = object(
                {
                    ...
                    virtualNetworkList = list(object(
                            {
                                vnetName = string
                                subnetName = string
                                subnetId = string
...
resource "azurerm_sql_active_directory_administrator" "aad_admin" {
    ...
    server_name         = "${var.environment.envPrefix}-${var.rg.domain}-sqlsrv"
    resource_group_name = "${var.environment.envPrefix}-${var.rg.domain}-sqlsrv-rg"
    login               = "${var.sqlServer.aad_admin.role_name}"
...
resource "azurerm_sql_firewall_rule" "fw_rule" {
    count = length(var.sqlServer.virtualNetworkRules.firewallRules)
    depends_on = [azurerm_sql_server.sqlsrv]

    name                = "${var.sqlServer.virtualNetworkRules.firewallRules[count.index].name}"
    resource_group_name = "${var.environment.envPrefix}-${var.rg.domain}-sqlsrv-rg"
    server_name         = "${var.environment.envPrefix}-${var.rg.domain}-sqlsrv"
    start_ip_address    = "${var.sqlServer.virtualNetworkRules.firewallRules[count.index].startIpAddress}"
    end_ip_address      = "${var.sqlServer.virtualNetworkRules.firewallRules[count.index].endIpAddress}"
}

resource "azurerm_sql_virtual_network_rule" "sqlvnetrule" {
    count = length(var.sqlServer.virtualNetworkRules.virtualNetworkList)
    depends_on = [azurerm_sql_server.sqlsrv]

    name                = "${var.sqlServer.virtualNetworkRules.virtualNetworkList[count.index].vnetName}"
    resource_group_name = "${var.environment.envPrefix}-${var.rg.domain}-sqlsrv-rg"
    server_name         = "${var.environment.envPrefix}-${var.rg.domain}-sqlsrv"
    subnet_id           = "${var.sqlServer.virtualNetworkRules.virtualNetworkList[count.index].subnetId}"
}
